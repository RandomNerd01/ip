        ...
        while (true) {
            ...
            try {
                ...
                if ("bye".equalsIgnoreCase(cmd)) {
                    ...
                    String keyword = meowthecat.CommandParser.parseFindQuery(line);
                    List<meowthecat.Task> matches = tasks.find(keyword);
                    ui.showFind(matches);
                } else {
...
class ConsoleUI {
     ...
     */
    void showFind(List<meowthecat.Task> matches) {
...
class CommandParser {
     ...
     * @throws meowthecat.MeowException if the query is empty
     */
    static String parseFindQuery(String line) throws meowthecat.MeowException {
        String rest = line.length() > 4 ? line.substring(4).trim() : "";
        if (rest.isEmpty()) {
            throw new meowthecat.MeowException("The find command requires a non-empty keyword.");
        }
        return rest;
    }
    /**
...
class TaskCollection {
     ...
     */
    List<meowthecat.Task> find(String keyword) {
        Objects.requireNonNull(keyword, "keyword must not be null");
        String lower = keyword.toLowerCase(Locale.ENGLISH);
        List<meowthecat.Task> results = new ArrayList<>();
        for (meowthecat.Task t : tasks) {
